...
describe('AcceptRelationRequest', ()=>{
  ...
  it('Should not touch product quantity of friend', async()=>{
    if(serviceSelect === ServerSource.fastify) {
      await app.ready();
    }
  ...
  it('Should not change product quantity of user', async()=>{
    if(serviceSelect === ServerSource.fastify) {
      await app.ready();
    }
    ...
    // Ajouts de 1 produits pour le userId = 2
    const product3 =  {
      userId: '2',
      openDate: new Date(),
      image: {
